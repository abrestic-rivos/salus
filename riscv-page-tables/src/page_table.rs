...
impl<T: PagingMode> GuestStagePageTable<T> {
    ...
    /// Unassigns pages that were previously mapped in this page table if they were invalidated a
    /// TLB version older than `tlb_version`. Guarantees that the entire range has been unassigned
    /// and the PTEs cleared upon success.
