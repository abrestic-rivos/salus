...
impl Imsic {
    ...
    /// Completes saving of `guest_file` to `sw_file`. The caller must ensure that any translations in
    /// the CPU or MSI page table referencing `guest_file` have been flushed. Upon return `guest_file`
    /// is available for reuse.
    pub fn save_guest_file_finsih(
        &self,
        guest_file: ImsicFileId,
        sw_file: &mut SwFile,
    ) -> Result<()> {
